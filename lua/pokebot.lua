...
console.log("Bot connected: " .. profile)
-- Allocate memory mapped file sizes
comm.mmfWrite("bizhawk_screenshot-" .. profile, string.rep("\x00", 24576))
comm.mmfSetFilename("bizhawk_screenshot-" .. profile)
comm.mmfScreenshot()

comm.mmfWrite("bizhawk_press_input-" .. profile, string.rep("\x00", 4096))
comm.mmfWrite("bizhawk_hold_input-" .. profile, string.rep("\x00", 4096))
comm.mmfWrite("bizhawk_trainer_data-" .. profile, string.rep("\x00", 4096))
comm.mmfWrite("bizhawk_bag_data-" .. profile, string.rep("\x00", 8192))
comm.mmfWrite("bizhawk_party_data-" .. profile, string.rep("\x00", 8192))
comm.mmfWrite("bizhawk_opponent_data-" .. profile, string.rep("\x00", 4096))
comm.mmfWrite("bizhawk_emu_data-" .. profile, string.rep("\x00", 4096))
...
comm.mmfWrite("bizhawk_hold_input-" .. profile, json.encode(input) .. "\x00")
comm.mmfWrite("bizhawk_input_list-" .. profile, string.rep("\x00", 4096))

comm.mmfWriteBytes("bizhawk_input_list-" .. profile, input_list)
...
function mainLoop()
	...
	comm.mmfWrite("bizhawk_trainer_data-" .. profile, json.encode({["trainer"] = trainer}) .. "\x00")
	comm.mmfWrite("bizhawk_party_data-" .. profile, json.encode({["party"] = party}) .. "\x00")
	comm.mmfWrite("bizhawk_opponent_data-" .. profile, json.encode({["opponent"] = opponent}) .. "\x00")
	comm.mmfWrite("bizhawk_bag_data-" .. profile, json.encode({["bag"] = bag}) .. "\x00")
...
function traverseNewInputs()
	local pcall_result, list_of_inputs = pcall(comm.mmfRead,"bizhawk_input_list-" .. profile, 4096)
...
function handleHeldButtons()
	...
	local pcall_result, hold_result = pcall(json.decode, comm.mmfRead("bizhawk_hold_input-" .. profile, 4096))
...
while true do
	...
	comm.mmfWrite("bizhawk_emu_data-" .. profile, json.encode({["emu"] = emu_data}) .. "\x00")
	fps = emu_data.fps
